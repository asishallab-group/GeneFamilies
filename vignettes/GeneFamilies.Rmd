...
Decide on which genome to anchor or reference the reciprocal Best Blast Hits. Then run pairwise sequence similarity searches, each in both directions, as in the following example. Here, we use Arabidopsis as anchor genome:

For comparing Arabidopsis and _C. hirsuta_:

`blat chi.cds.fa ath.cds.fa -out=blast8 ath_vs_chi_blat_out.tsv`
`blat ath.cds.fa chi.cds.fa -out=blast8 chi_vs_ath_blat_out.tsv`

Now collect each best non-self match for each single Arabidopsis gene:

`cat chi_vs_ath_blat_out.tsv | sort -k1,1 -k3,3nr -k12,12nr | awk -F "\t" 'BEGIN{x=""}{if(x != $1){x=$1; print $2 "\t" $1}}' | sort -u > chi_vs_ath_blat_out_only_best_non_selves.tsv`

`cat ath_vs_chi_blat_out.tsv | sort -k1,1 -k3,3nr -k12,12nr | awk -F "\t" 'BEGIN{x=""}{if(x != $1){x=$1; print $1 "\t" $2}}' | sort -u > ath_vs_chi_blat_out_only_best_non_selves.tsv`

Note, the _reversed_ order in the `awk` `print` statements.

Now, select the lines (gene-pairs) that both files have in common:

`comm -1 -2 ath_vs_chi_blat_out_only_best_non_selves.tsv chi_vs_ath_blat_out_only_best_non_selves.tsv > ath_chi_orthologs.tsv`

Do the above for _all_ other pairs _with_ Arabidopsis (anchor genome).

Finally concatonate all the above `ath_*_orthologs.tsv` files into a single one, sorted by the first column and use this package's R script to load the orthologs and do the final processing step.

`cat ath_*_orthologs.tsv | sort -k1,1 > spe8_orthologs.tsv`

#### Identification of Tandem Duplicated Genes

We identify tandem duplicated genes ("Tandems") as genes belonging to the same species, being located on the same chromosome within a a maximum distance of _nine_ genes, and having significant sequence similarity to each other. For reasons of convenience the last information is obtained as genes belonging to the same gene family.

Many methods exist to infer gene neighborhood based on GFF3 files. However, experience showed that often the files do not stick fully to the required format, especiallyreliable extraction of gene identifier can be problematic. Hence, we use a somewhat crude method relying on GNU shell commands.

We need the GFF3 files for each investigated species. They need to be sorted by _chromosome_ or _scaffold_ and the gene position. Lastly the gene identifiers need to be extracted. Depending on the provided GFF3 file, you will need to adapt the following example commando:

`awk -F "\t" '/^[^#]/{if($3 =="gene"){gsub(/(^ID=)|(;Name=.*$)/,"",$9); print $1 "\t" $4 "\t" $9}}' Brapa_gene_v1.5.gff | sort -k1,1 -k2,2n | awk -F
"\t" '{print $1 "\t" $3}' > bra_gene_neighborhood.tsv`

or in the case of _A. lyrata_ the gene IDs need a different regular expression:

`awk -F "\t" '/^[^#]/{if($3 =="gene"){gsub(/(^ID=)|(\.v1;Name=.*$)/,"",$9); print $1 "\t" $4 "\t" $9}}' Alyrata_107_v1.0.gene.gff3 | sort -k1,1 -k2,2n | awk -F "\t" '{print $1 "\t" $3}' > aly_gene_neighborhood.tsv`
