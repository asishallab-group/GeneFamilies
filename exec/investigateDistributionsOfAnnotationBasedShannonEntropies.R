...
#' gene-groups:
pdf(file.path(input.args[[1]], "inst", "functionDiversitiesPerGeneGroupClassesBoxplot.pdf"))
plot.lst <- list(Orth = orthologs.ipr.entropies, Tand = tandems.ipr.entropies, Exp = families.ipr.entropies[families.exp], 
    PSel = families.ipr.entropies[fams.pos.sel])
...
plot.df$Group.Class <- factor(plot.df$Group.Class, levels = c("Orth", "Tand", "Exp", 
    "PSel"))
b.cols <- brewer.pal(length(plot.lst), "Dark2")
boxplot(Entropy ~ Group.Class, data = plot.df, outpch = "-", cex = 0.5, border = b.cols, 
    col = addAlpha(b.cols), lwd = 2.5)
dev.off()


#' Plot the same, but comparing within each group 'background' orthologs with
#' non-orthologs:
oplot.lst <- list(Tand = tandems.ipr.entropies.orth.dist, Exp = families.ipr.entropies.orth.dist[which(families.ipr.entropies.orth.dist$Family %in% 
...
oplot.df <- Reduce(rbind, lapply(names(oplot.lst), function(x) {
    ...
    y.df <- rbind(data.frame(Entropy = x.df$Orthologs, Gene.Class = "Orthologs", 
        stringsAsFactors = FALSE), data.frame(Entropy = x.df$"Non-Orthologs", Gene.Class = "Non-Orthologs", 
...
g.pl <- ggplot(oplot.df, aes(y = Entropy, x = Group.Class, fill = Gene.Class)) + 
    geom_boxplot(outlier.shape = "-", outlier.size = 1) + theme_bw() + theme(plot.background = element_blank(), 
...
ggsave(plot = g.pl, filename = file.path(input.args[[1]], "inst", "functionDiversitiesPerGeneGroupClassesAndOrthologDistinctionBoxplot.pdf"))


#' Do t-Tests for each group, testing the alternative hypothesis that orthologs
#' have lower diversities than non-orthologs:
ipr.entropies.orth.dist.t.tests <- Reduce(rbind, lapply(names(oplot.lst), function(x) {
    x.df <- oplot.lst[[x]]
    t.h.test <- t.test(x.df[, "Orthologs"], x.df[, "Non-Orthologs"], alternative = "less")
...
#' Save results:
write.table(ipr.entropies.orth.dist.t.tests, file = file.path(input.args[[1]], "inst", 
    "functionDiversitiesPerGeneGroupClasses_t-tests.tsv"), sep = "\t", quote = FALSE, 
    row.names = FALSE)
write.table(ipr.entropies.orth.inter.group.class.t.tests, file = file.path(input.args[[1]], 
    "inst", "functionDiversitiesInterGeneGroupClasses_t-tests.tsv"), sep = "\t", 
